...
blue_new_project_task = Blueprint('blue_new_project_task', __name__)


def init_new_project_task(app):
    app.register_blueprint(blueprint=blue_new_project_task)



@blue_new_project_task.route("/page/v0/new_project_task/", methods=['GET'])
def new_project_task_index():

    #leader = request.args.get('leader')
    form_no = request.args.get('form_no')
    project_no = request.args.get('project_no')
    mode = 'new_project_task'
...
@blue_new_project_task.route("/api/v0/db/new_project_task/", methods=['post'])
...
@blue_new_project_task.route("/api/v0/db/new_project_task/by/project_no/", methods=['post'])
def get_new_project_task():
...
@blue_new_project_task.route("/api/v0/db/new_project_task/by/project_no/lastest/", methods=['get'])
...
# 指派工作送出指派通知信
@blue_new_project_task.route("/api/v0/send/new_project_task/", methods=['post'])
def send_require_email_by_project_no():
    ...
    subject  =  '任務指派通知-{}'.format(product_no)
   
    message  =  '<!DOCTYPE html>' + \
                '<html leng="en">' + \
                '<head><meta charset="utf-8"></head>' + \
                '<body>' + \
                '<h1>任務指派通知-{}</h1>'.format(product_no) + \
                '<ul>' + \
                ...
                '</ul><br/>' + \
                '<a href="http://{}/page/v0/new_project_display/?project_no={}">新產品開案申請單</a>'.format(DOMAIN_PATH,project_no)+ \
                '</ul><br/>' + \
                '<a href="http://{}/page/v0/new_project_feedback/?project_no={}">新產品開案回報清單</a>'.format(DOMAIN_PATH,project_no)+ \
                '</body>' + \
                '</html>'
    print('---------------')
    print(sender)
...
@blue_new_project_task.route("/api/v0/send/new_project_task/layout/", methods=['post'])
def send_assign_email_by_layout():
    ...
    message  =  '<!DOCTYPE html>' + \
                ...
                '<li>預計結束日期: {}</li>'.format(project_end) + \
                '</ul><br/>' + \
                '<a href="http://{}/page/v0/layout_display/?project_no={}&form_no={}&status=1">Layout申請工程規格聯絡單</a><br/><br/>'.format(DOMAIN_PATH,project_no,form_no)+ \
                '</ul><br/>' + \
                '<a href="http://{}/page/v0/new_project_feedback/?project_no={}">新產品開案回報清單</a>'.format(DOMAIN_PATH,project_no)+ \
                '</body>' + \
                '</html>'
...
# 指派工作送出指派通知信(pcb表單指派)
@blue_new_project_task.route("/api/v0/send/new_project_task/pcb/", methods=['post'])
def send_assign_email_by_pcb():
    ...
    message  =  '<!DOCTYPE html>' + \
                ...
                '<li>預計結束日期:{}</li>'.format(project_end) + \
                '</ul><br/>' + \
                '<a href="http://{}/page/v0/pcb_display/?project_no={}&form_no={}&status=1">PCB打樣申請工程規格聯絡單</a>'.format(DOMAIN_PATH,project_no,form_no)+ \
                '</ul><br/>' + \
                '<a href="http://{}/page/v0/new_project_feedback/?project_no={}">新產品開案回報清單</a>'.format(DOMAIN_PATH,project_no)+ \
                '</body>' + \
                '</html>'
...
@blue_new_project_task.route("/api/v0/db/new_project_task/update_status/", methods=['get'])
...
@blue_new_project_task.route("/api/v0/db/new_project_task/update_memberlist/", methods=['post'])
